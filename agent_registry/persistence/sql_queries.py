...
class SQLiteQueries(str, Enum):
    """SQL queries for SQLite backend (placeholder: ?, JSON stored as TEXT)."""
    ...
    FIND_BY_NAME = """
        SELECT agent_card_json FROM agent_card WHERE name LIKE ?
    """

    FIND_BY_ORG = """
        SELECT agent_card_json FROM agent_card WHERE organization = ?
    """
    ...
    UPDATE_STATUS = """
        UPDATE agent_card SET status = ?, updated_at = ?
        WHERE name = ? AND organization = ?
    """

    DELETE_AGENT = """
        DELETE FROM agent_card WHERE name = ? AND organization = ?
    """
    ...
    GET_AGENT_TAGS = """
        SELECT tags FROM agent_card WHERE name = ? AND organization = ?
    """

    UPDATE_AGENT_TAGS = """
        UPDATE agent_card SET tags = ?, updated_at = ?
        WHERE name = ? AND organization = ?
    """
    ...
    UPDATE_TAG = """
        UPDATE tag SET name = ?, updated_at = ?
        WHERE tag_id = ?
    """
...
class GaussDBQueries(str, Enum):
    """SQL queries for GaussDB backend (placeholder: %s, JSON stored as TEXT).

    GaussDB is PG-protocol compatible. JSON columns use TEXT for maximum
    portability. Cast to jsonb at query time for JSON operators.
    """
    ...
    ADD_COLUMN_TAGS = """
        ...
        BEGIN
            IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                           WHERE table_name='agent_card' AND column_name='tags') THEN
                ALTER TABLE agent_card ADD COLUMN tags TEXT DEFAULT '[]';
            END IF;
        END $$;
    """
