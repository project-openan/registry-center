...
所有SQL后端共享同一套CRUD引擎（`agent_registry/persistence/sql_backend.py`），各自的方言SQL集中在 `agent_registry/persistence/sql_queries.py`。新增一种数据库需要：一个查询枚举、一个 `SqlStorageBackend` 子类、`agent_registry/persistence/__init__.py` 工厂中的一个分支，以及 `persistence.conf` 中的一段配置。
...
配置文件加载时解析 `${ENV:default}` 占位符，任意配置项都可通过环境变量覆盖而无需修改文件。容器化部署中，`bin/entrypoint.sh` 会将通用 `DB_*` 环境变量（`DB_HOST`、`DB_PORT`、`DB_NAME`、`DB_USERNAME`、`DB_PASSWORD`、`DB_POOL_MIN`、`DB_POOL_MAX`）改写到当前后端对应的配置项（支持 postgresql/gauss/mysql）；sqlite 原生支持 `SQLITE_PATH` 环境变量。
